...
F_CREATURE_DIR = os.path.join("assets", "svg_images")

F_CREATURE_SCALE_FACTOR = 0.5
...
class FCreature(SVGMobject):
    CONFIG = {
        "color": RED,
        "file_name_prefix": "FCreature",
        "stroke_width": 0,
        "stroke_color": BLACK,
        "fill_opacity": 1.0,
        "height": 3,
    ...
    def align_data(self, mobject):
        ...
        if isinstance(mobject, FCreature):
    ...
    def make_eye_contact(self, f_creature):
        self.look_at(f_creature.eyes)
        f_creature.look_at(self.eyes)
        return self
...
def get_all_f_creature_modes():
    result = []
    prefix = "%s_" % PiCreature.CONFIG["file_name_prefix"]
    suffix = ".svg"
    for file in os.listdir(F_CREATURE_DIR):
...
class PiCreatureBubbleIntroduction(AnimationGroup):
    ...
    def __init__(self, f_creature, *content, **kwargs):
        digest_config(self, kwargs)
        bubble = f_creature.get_bubble(
            *content,
        ...
        f_creature.generate_target()
        f_creature.target.change_mode(self.target_mode)
        if self.look_at_arg is not None:
            f_creature.target.look_at(self.look_at_arg)

        change_mode = MoveToTarget(f_creature, **self.change_mode_kwargs)
...
class FCreatureSays(PiCreatureBubbleIntroduction):
    CONFIG = {
        "target_mode": "speaking",
        "bubble_class": SpeechBubble,
    }
...
class Blink(ApplyMethod):
    ...
    def __init__(self, f_creature, **kwargs):
        ApplyMethod.__init__(self, f_creature.blink, **kwargs)
